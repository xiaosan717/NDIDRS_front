  ...
  <div class="hazard-handle">
    ...
    <div class="handle-table">
      ...
      <div v-for="hazard in pendingHazards" :key="hazard.id" class="table-row">
        <span>{{ hazard.studentName || hazard.studentId }}</span>
        <span>{{ hazard.roomNumber || hazard.roomId }}</span>
        <span>{{ getTypeText(hazard.hazardType) }}</span>
        <span>{{ hazard.description }}</span>
        <span class="actions">
          <template v-if="isDormManager">
            <button class="action-btn complete" @click="handleApprove(hazard)">{{ t('hazardHandle.approve') }}</button>
            <button class="action-btn reject" @click="handleReject(hazard)">{{ t('hazardHandle.reject') }}</button>
          </template>
          <template v-else-if="isAdmin">
            <button class="action-btn" @click="handleProcess(hazard)">{{ t('hazardHandle.processing') }}</button>
            <button class="action-btn complete" @click="handleComplete(hazard)">{{ t('hazardHandle.complete') }}</button>
            <button class="action-btn reject" @click="handleReject(hazard)">{{ t('hazardHandle.reject') }}</button>
          </template>
        </span>
      </div>
